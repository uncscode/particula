...
Workflows adopt the branch tiers defined in the E11 epic; canonical source:
...
Use these patterns when selecting `--base`, applying `branch:*` labels, or designing workflows
that create worktrees/PRs:

- `develop`: trunk for stabilization work (`ADW_DEV_BRANCH` can override the default).
- `epic{NN}-{adw_id}-{slug}`: cross-cutting epic threads branched from develop.
- `feat{NN}-{adw_id}-{slug}`: standalone features branched from develop.
- `{type}-{issue}-adw-{id}-{slug}`: issue-linked branches retained for backward-compatible/main
  workflows.

See **[E11-F4 – Worktree Base Branch & naming rollout](https://github.com/Gorkowski/Agent/blob/main/adw-docs/dev-plans/features/completed/E11-F4-worktree-base-branch.md)**
(Completed) for the compact table aligned to the epic.

### Auto-Mode Guardrails

Auto-mode dispatch requires a resolvable branch name and a non-protected base. Guardrails
enforce the following:

- Detached HEAD states are not supported; if the current branch cannot be determined, the
  dispatcher blocks auto-mode and avoids mutating the manifest.
- `main`/`master` branches are blocked to protect production history.
- Label gating is enforced only when the platform client supports `Operation.ISSUE_READ`;
  if label reads are unavailable, the scheduler fails open to the first ready issue to
  preserve prior automation behavior during dispatch. Resume from a paused manifest still
  requires issue read access—if unavailable, auto-mode remains paused until permissions
  are restored.

CI gate evaluation can mark a running issue as `failed` when checks conclude with
failures. The manifest records fix-tracking metadata (`fix_attempt_count`,
`fix_adw_id`, `last_failure_at`, `failure_reason`) and uses the `fixing` state when a
fix workflow is in progress.

Accumulate-mode finalization opens a single final PR via `open_final_pr()` and
posts a deterministic `Final Handoff — Branch Accumulation Complete` comment on
that PR. Runtime posting uses bounded duplicate checks and now treats
not-posted/error outcomes as `Blocked` so finalization does not report false
success.

Recovery: if the manifest is corrupted or cannot be loaded, delete
`.adw/auto_mode_manifest.json` (and its lock file if present) and re-run
`adw auto-mode init` to rebuild a clean manifest.

See also: **[Architecture Reference](architecture_reference.md#auto-mode-architecture)**.

## CLI Reference

### Built-in Auto Variants

In addition to `complete` and `patch`, ADW includes auto variants:

- `complete-auto`
- `patch-auto`

Both variants keep the same base pipeline as their standard counterparts, but:

- use `ship-auto` for the Ship step
- add a conditional Fix agent step that runs only when `state.request_fix == True`
- skip that Fix step when `state.fix_completed == True`
...
- complete-auto: Full validation workflow with auto ship/fix gating
- patch-auto: Quick patch workflow with auto ship/fix gating
- test: Test workflow
...
# Run complete auto workflow (ship-auto + conditional Fix step)
adw workflow complete-auto 123
...
### Platform-agnostic status and TODO synchronization

- Preferred imports: `from adw.workflows.operations import status, todo_sync`
  (or import functions directly). Platform-layer shims are available at
  `adw.platforms.status` and `adw.platforms.todo_sync`.
- `adw.github.status` and `adw.utils.todo_sync` are deprecated shims that emit
  `DeprecationWarning` while delegating to the workflow operations modules. Migrate
  callers to the workflow ops imports; only use the platform shims when layering requires
  platform namespaces.
- Status and TODO comments call `PlatformRouter` with the persisted
  `status_comment_id` + scope, reuse stored scope on updates, fall back to create when an
  update fails, and skip no-op bodies. The bot identifier is centralized at
  `adw.core.constants.ADW_BOT_IDENTIFIER`.
- CI gate failures post a dedicated PR comment that is **upserted** (updated in place)
  using a marker (`<!-- adw-ci-check-status -->`) so repeated runs replace the previous
  summary instead of creating duplicates. The comment includes a concise markdown table
  of failing checks and a short operator guidance line when failures are present.

## Troubleshooting

### Auto-mode dispatcher state persistence

Auto-mode performs a pre-dispatch state save before routing each issue workflow.

- On transient `FileNotFoundError` path races, dispatcher retries a bounded number
  of times.
- If those retries are exhausted, dispatcher logs a degraded-mode warning and
  continues dispatch.
- Non-`FileNotFoundError` persistence failures are treated as hard-fail errors and
  abort dispatch for that issue.
...
**Review agent output:**
```bash
cat agents/<adw-id>/<agent-name>/raw_output.log
```
