...
**New in 2.3.0:** Added `fix` workflow type metadata with optional `trigger` (label-based, draft exclusion) and `on_failure` handling, plus per-agent `constraints` for tool permissions.
...
| `workflow_type` | string | No | `null` | Workflow type: `complete`, `patch`, `document`, `generate`, `fix`, or `custom` |
| `trigger` | object | No | `null` | Optional trigger metadata with `label` (string, min length 1) and `exclude_draft` (boolean, default `false`). |
| `on_failure` | object | No | `null` | Optional failure handling metadata containing `labels` (array of strings) and optional `comment` (string). |
...
Optional metadata describing when a workflow should run automatically.

| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `label` | string | Yes* | - | Label required to trigger workflow execution. Min length: 1. |
| `exclude_draft` | boolean | No | `false` | When `true`, drafts are excluded from triggering. |

\*Required only when a `trigger` object is present.

### On Failure

Optional metadata to apply when a workflow fails.

| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `labels` | array | No | `[]` | Labels to add when workflow fails. |
| `comment` | string | No | `null` | Comment body to post on failure. |
...
- **Trigger:** `label: request:fix`, `exclude_draft: true`
- **On failure:** add `fix-failed` label and optional failure comment
- **Steps:** init → fetch-comments → analyze → fix → lint → test → commit → complete (agent/workflow types only)
- **Constraints:** `fix`, `lint`, and `test` steps typically allow `read`, `edit`, `write`, atomic git wrappers (`git_diff`, `git_stage`, `git_commit`, `git_branch`, `git_merge`, `git_worktree`), `run_linters`, `run_pytest` and deny `bash`
...
| `constraints` | object | No | `null` | Tool permissions per agent. `allowed_tools` enum includes `read`, `edit`, `write`, atomic git wrappers (`git_diff`, `git_stage`, `git_commit`, `git_branch`, `git_merge`, `git_worktree`), `run_linters`, `run_pytest`; `denied_tools` defaults to `["bash"]`. |
...
|-------|------|----------|-------------|
| `fail_if` | string | No | Expression that fails the step when True. Evaluated first. |
| `fail_message` | string | No | Optional custom error text used when `fail_if` evaluates to True. |
| `skip_if` | string | No | Expression that causes step to be skipped if True. Evaluated after `fail_if`. |
| `if_condition` | string | No | Expression that must be True for step to execute. Evaluated after `fail_if`/`skip_if`. |

`fail_if`, `skip_if`, and `if_condition` can be combined. Evaluation order is deterministic:
1. `fail_if` (if True, fail immediately)
2. `skip_if` (if True, skip)
3. `if_condition` (if False, skip)
...
|-------|------|-------------|
| `state.workflow_type` | string | Workflow type: `complete`, `patch`, `fix`, `document`, `generate`, `custom` |
...
| `state.review_feedback` | string/null | Persisted slice review feedback (`null` when unset) |
| `state.request_fix` | boolean | Fix gate flag set by review/follow-up logic |
| `state.fix_completed` | boolean | Tracks whether the gated fix pass already ran |
...
**Fail step when required state is missing:**
```json
{
  "condition": {
    "fail_if": "state.spec_content == null",
    "fail_message": "Plan step did not produce spec_content"
  }
}
```
...
- **Workflow Migration Guide** (archived; see `.trash/adw-docs/workflow-migration-guide.md`) - Migrate Python workflows
