...
The `setup_update` agent automates ADW adoption and version updates by comprehensively analyzing repository conventions (testing, linting, code style, docs, architecture, commits, PRs), filling template guides in `adw-docs/`, and presenting findings to users for confirmation before writing changes.
...
- **ADW Version Updates**: You're updating existing `adw-docs/` guides to match new ADW template versions
...
- **Read Access**: 
  ...
  - Existing documentation (`adw-docs/`, `README.md`, `CONTRIBUTING.md`)
...
- **Existing Guides** (Update mode): `adw-docs/` - Current repository-specific guides
- **Repository Context**: Source files, configs, CI/CD workflows for convention detection

The agent writes to:
- **Active Guides**: `adw-docs/` - Repository-specific guides filled with detected conventions
...
1. **Mode Detection**: Agent detects no `adw-docs/` directory → Setup mode
...
4. **Confirmation Questions**:
   ...
   ```
5. **Create Guides**:
   - Copies templates from `adw/templates/Agent/` to `adw-docs/`
   - Replaces `{{PACKAGE_NAME}}`, `{{TEST_FRAMEWORK}}`, etc. with detected values
   - Saves each guide incrementally:
     ```
     ✓ Created adw-docs/testing_guide.md
     ✓ Created adw-docs/linting_guide.md
     ✓ Created adw-docs/code_style.md
     ...
     ```
...
- `adw-docs/commit_conventions.md`
- `adw-docs/pr_conventions.md`
...
1. **Mode Detection**: Agent detects existing `adw-docs/` directory → Update mode
...
3. **Compare Guides**:
   - For each guide, shows unified diff:
     ...
     ════════════════════════════════════════════════════════
     --- Current: adw-docs/testing_guide.md
...
**Files Updated**:
- 7 guides in `adw-docs/` (updated to v2.1 templates)
...
**Files Written**:
- `adw-docs/` guides filled with JavaScript/TypeScript conventions
...
- **Sync Command** (`/sync`): While `setup_update` fills `adw-docs/` guides, the `/sync` command keeps `.opencode/command/` command files in sync with `adw/commands/` templates. Run both for complete ADW setup.

- **Implementor Agent**: After setup, the implementor agent uses `adw-docs/` guides to understand repository conventions when implementing features.

- **Review Agent**: Review agent references `adw-docs/review_guide.md` filled by `setup_update` to enforce quality standards.
...
✗ Placeholders remaining:
   - adw-docs/architecture_reference.md:15 - {{ARCHITECTURE_DOC_LINK}}
...
- Or: Review diff in external tool:
  ```bash
  diff adw-docs/testing_guide.md adw/templates/Agent/testing_guide.md
  ```
...
- **Guide Documentation**: `adw-docs/README.md` - Overview of all guides (created by setup_update agent)
