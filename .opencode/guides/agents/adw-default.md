...
The `adw_default` agent is the general-purpose fallback agent for the ADW (Agent Developer Workflow) system. It handles custom slash commands, serves as a fallback when specialized agents are not needed, and provides comprehensive access to ADW tools and repository context.
...
- **`adw-docs/README.md`** - Complete agent documentation index
...
- `get_datetime` - Timestamps for logs (UTC or America/Denver via `localtime`)
...
2. Read `adw-docs/architecture_reference.md` for details
...
See `adw-docs/architecture_reference.md` for complete details.
```
...
- `adw-docs/README.md` - Agent documentation index
...
- Testing: pytest, `*_test.py` suffix, 80% coverage minimum
...
- **Agent Documentation Index**: `/home/kyle/Code/Agent/adw-docs/README.md` - All guides
- **Architecture Reference**: `adw-docs/architecture_reference.md` - System design
- **Code Style Guide**: `adw-docs/code_style.md` - Python conventions
- **Testing Guide**: `adw-docs/testing_guide.md` - Test framework and patterns
