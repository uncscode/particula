...
## Atomic git wrappers: worktree management

The `git_worktree` tool exposes worktree lifecycle commands that map directly to
`adw git worktree`:

- `worktree-list` → `adw git worktree list`
- `worktree-prune` → `adw git worktree prune`
- `worktree-remove` → `adw git worktree remove <adw_id> [--force]` (requires `adw_id`; force
  is enabled by default in the wrapper, and `force: false` omits `--force` so git can block
  removal when state is dirty).

Use the `help` flag to bypass validation and view CLI help output. Successful calls return git
stdout; errors include stderr with the `ERROR:` prefix.

### Commit failure diagnostics (tool wrapper)

For `git_commit` failures, wrapper responses now include a structured `ERROR:`
envelope with deterministic diagnostics to speed up triage:

- command context (commit-specific inputs such as `worktree_path`/staging mode)
- exit code
- bounded stderr/stdout snippets (with explicit truncation marker when clipped)
- hint mapping with deterministic precedence

This behavior is wrapper-side and does not change git commit semantics; it improves the quality
and consistency of failure output returned to agents/operators.

### Commit success markers (`git_commit`)

Successful `git_commit` responses now expose parseable outcome markers:

- committed result: `status=committed` and `sha=<full-head-sha>`
- no-op result: `status=no_op` and no `sha` marker

Use these markers when downstream automation needs to distinguish a real new
commit from a clean-tree no-op path.

### Optional commit hook bypass (`no_verify`)

The `git_commit` wrapper supports an optional commit-only `no_verify`
boolean:

- `no_verify: true` maps to `git commit --no-verify`.
- `no_verify: false` (or omission) does not append `--no-verify`.
- Non-`commit` commands are unaffected.

Use this only when you intentionally need to bypass local commit hooks.
...
await git_worktree({
  ...
  adw_id: "abc12345",
  force: false, // use true (default) to force removal
});
```
...
- Wrapper path validation enforces canonical, in-repo boundaries for guarded operations and
  rejects unsafe/path-injection style inputs with deterministic `ERROR:` envelopes.
...
### Before (Basic Usage)
```python
response = execute_opencode_agent("Implement feature", "abc12345")
```
