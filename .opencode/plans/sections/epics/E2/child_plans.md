...
| E2-F2 | CPU `EnvironmentData` container | Draft | Add per-box temperature, pressure, species-resolved `saturation_ratio`, validation, and docs. |
...
| E2-F8 | CPU dynamics container boundaries | Draft | Clarify where data containers are supported and require explicit errors for unsupported multi-box CPU paths. |
