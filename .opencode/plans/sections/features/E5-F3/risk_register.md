...
| Accepted execution loses charge or leaves donor state active | Low | Critical | Reuse E5-F2 merge once; assert separate per-box mass/charge conservation and donor clearing | E5-F3-P2/P3 |
| Capability validation or allocations mutate output/RNG state before failure | Low | High | Register capability only after prerequisites; snapshot all caller-owned state for failures | E5-F3-P2 |
...
| Stochastic tests are flaky or require exact CPU/Warp replay | Medium | Medium | Use fixed seed sets with independent aggregate/sigma bounds and deterministic structural/conservation checks | E5-F3-P2/P3 |
