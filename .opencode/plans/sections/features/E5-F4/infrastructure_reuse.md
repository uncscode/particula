...
- `get_sedimentation_kernel_sp2016()` in
  `particula/dynamics/coagulation/sedimentation_kernel.py:61-122` is the
...
- `get_particle_settling_velocity()` and
  ...
  `particula/particles/properties/settling_velocity.py:39-99` and `:177-244`
  define the approved Stokes, gas-property, Knudsen, and slip-correction path.
- `brownian_coagulation_kernel()` in
  `particula/gpu/kernels/coagulation.py:169-427` already computes composition
...
- `coagulation_step_gpu()` in
  `particula/gpu/kernels/coagulation.py:792-1022` is the concrete low-level
...
- `_bound_scheduled_trials()` and active-index swap-pop helpers in
  `particula/gpu/kernels/coagulation.py:66-119` provide the bounded sampling and
...
- `particula/gpu/dynamics/coagulation_funcs.py:10-139` establishes the pattern
  for small scalar `@wp.func` pair/property helpers with independent CPU
  references and focused probe-kernel tests.
- `particula/gpu/kernels/tests/coagulation_test.py:263-296` provides device
