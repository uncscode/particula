...
- E2-F2 depends on T1 for the future `EnvironmentData` field list, shapes, and
  ownership rules.
- E2-F3 depends on T1 for `WarpEnvironmentData` mirroring and conversion
  semantics.
- E2-F4 depends on T1 for gas versus Warp gas ownership, especially names,
  partitioning, and vapor pressure.
- E2-F5 depends on T1 for scalar-to-per-box compatibility rules.
- E2-F6 through E2-F8 depend on T1 for dtype and support-boundary language.
- E2-F9 depends on T1 for final user-facing data-model documentation.
