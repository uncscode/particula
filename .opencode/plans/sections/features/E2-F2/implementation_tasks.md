...
- Add fields for temperature, pressure, and humidity/saturation state using
  `NDArray[np.float64]` type hints.
- Coerce inputs with `np.asarray(..., dtype=np.float64)` in `__post_init__`.
- Validate one-dimensional shape and equal `(n_boxes,)` length for all fields.
...
- Update data-container migration docs to include `EnvironmentData` as the
  owner of per-box thermodynamic state.
