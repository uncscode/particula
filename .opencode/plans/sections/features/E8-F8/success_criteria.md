...
- [ ] The changed-module coverage list is frozen from shipped implementation
  ...
  per-target result.
- [x] Parent/sibling labels consistently assign profiling to E8-F7 and the T8
...
- [ ] P4 documentation reconciliation remains in progress. Current focused,
  active split-plan, and `mkdocs build --strict` evidence must be recorded
  consistently before completion. This is documentation-only, non-promoting
  evidence
  and is not CUDA evidence.
- [ ] P3's `UNSHIPPED/BLOCKED` ledger remains non-promoting until E8-F2,
  E8-F3, E8-F6,
