...
The direct particle-resolved GPU coagulation path ships the exact
Saffman-Turner 1956 (ST1956) turbulent-shear singleton. Turbulent dissipation
and carrier-fluid density can differ by simulation box, so explicit ownership
of those inputs avoids ambiguous scalar assumptions or pressure-derived density.
...
finite Python `float` or NumPy floating scalar, or same-device `wp.float64`
`(n_boxes,)` dissipation and
...
E5-F5-P1 is complete: internal fp64 Warp helpers provide kinematic viscosity
(`mu / rho`) and the ST1956 pair rate in
`particula/gpu/dynamics/coagulation_funcs.py`. The shipped direct-step contract
remains limited to the exact particle-resolved singleton; it adds no public
re-export, CPU fallback, runnable API, turbulent combinations, or general
turbulence accuracy claim.
