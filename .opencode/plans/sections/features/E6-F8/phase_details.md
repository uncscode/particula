...
- [ ] **E6-F8-P2:** Implement device nucleation rate and inventory finalization with unit tests
  - Issue: TBD | Size: S | Status: Not Started
  - Goal: Compute potential events and one shared gas-feasible admission factor per box before any source or gas write.
  - Files: `particula/gpu/kernels/nucleation.py`, `particula/gpu/kernels/tests/nucleation_test.py`
  - Tests: Activation/kinetic parity, each limiting species, zero inventory/rate/time, nonnegative gas, and finalized diagnostics.

- [ ] **E6-F8-P3:** Integrate fixed-shape slot activation sidecars with unit tests
  - Issue: TBD | Size: S | Status: Not Started
  - Goal: Package finalized sources into E6-F5 requests and activate ascending free slots without shape or identity changes.
...
- [ ] **E6-F8-P4:** Integrate device exhaustion planning without fallback with unit tests
  - Issue: TBD | Size: S | Status: Not Started
  - Goal: Consume E6-F6 resampling-first/scaling-fallback plans for complete finalized demand and reject unsatisfied plans before writes.
  - Files: `particula/gpu/kernels/nucleation.py`, E6-F6 exhaustion module, GPU kernel tests
  - Tests: Full slots, policy combinations, precedence, insufficient scratch, unsatisfiable demand, no truncation, and conservation snapshots.
