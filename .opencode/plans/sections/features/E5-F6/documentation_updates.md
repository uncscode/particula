...
- Update the `coagulation_step_gpu` and mechanism-configuration docstrings in
  ...
  semantics.
- Update `docs/Features/data-containers-and-gpu-foundations.md` with caller
  ownership of additive inputs, one output pair buffer, one persistent per-box
  RNG stream, and charge-aware merge behavior.
- Update `docs/Features/Roadmap/data-oriented-gpu.md` with E5-F6 status,
  supported two-way/full four-way evidence, explicit unsupported rows, and the
  E5-F7/E5-F9 handoff.
- Record verified support boundaries in the E5 epic and E5-F6 plan sections;
  do not mark E5-F7's cross-mechanism matrix or E5-F9's final example complete
  from this track.
- Provide E5-F9 with concise example requirements: direct low-level import,
...
- Update `AGENTS.md` only if the shipped low-level usage contract is important
  enough for the repository quick reference; no new agent/workflow is added.
