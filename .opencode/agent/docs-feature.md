---
description: >-
  Subagent that manages feature documentation in docs/Agent/feature/. Invoked
  by the documentation primary agent to create and update feature documentation
  following the established template format.
  
  This subagent:
  - Loads workflow context from adw_spec tool
  - Creates new feature docs following template.md format
  - Updates existing feature docs when features change
  - Archives completed/deprecated feature docs to archive/
  - Maintains docs/Agent/feature/README.md index
  - Ensures feature docs stay current with implementation
  - Validates markdown links
  
  Write permissions:
  - docs/Agent/feature/*.md: ALLOW
  - docs/Agent/feature/archive/*.md: ALLOW
mode: subagent
tools:
  adw_spec: true
  read: true
...
Create and update feature documentation in docs/Agent/feature/ following the established template format.

# Core Mission

Maintain comprehensive feature documentation with:
- New feature docs following template.md format
...
task({
  ...
  "prompt": f"Document new feature in docs/Agent/feature/.\n\nArguments: adw_id={adw_id}\n\nFeature: {issue_title}\nDetails: {issue_body}",
...
- @docs/Agent/feature/template.md - Feature doc template
- @docs/Agent/feature/README.md - Feature docs index
- @docs/Agent/documentation_guide.md - Documentation standards

# Write Permissions

**ALLOWED:**
- ✅ `docs/Agent/feature/*.md` - Create and update feature docs
- ✅ `docs/Agent/feature/README.md` - Update index
- ✅ `docs/Agent/feature/archive/*.md` - Archive completed/deprecated features
...
todowrite({
  "todos": [
    {
      "id": "1",
      "content": "Read feature template (docs/Agent/feature/template.md)",
      "status": "pending",
      "priority": "high"
    },
    {
    ...
    {
      "id": "3",
      "content": "Update docs/Agent/feature/README.md index",
      "status": "pending",
      "priority": "high"
    },
    {
...
```python
read({"filePath": "{worktree_path}/docs/Agent/feature/template.md"})
```
...
```python
write({
  "filePath": "{worktree_path}/docs/Agent/feature/{feature-name}.md",
...
Read existing doc:
```python
read({"filePath": "{worktree_path}/docs/Agent/feature/{existing-feature}.md"})
```
...
```python
edit({
  "filePath": "{worktree_path}/docs/Agent/feature/{feature}.md",
...
```bash
# Move to archive
mv docs/Agent/feature/{old-feature}.md docs/Agent/feature/archive/
```

Update the archived doc's status:
```python
edit({
  "filePath": "{worktree_path}/docs/Agent/feature/archive/{old-feature}.md",
...
Read current index:
```python
read({"filePath": "{worktree_path}/docs/Agent/feature/README.md"})
```

Add new feature to appropriate section:
```python
edit({
  "filePath": "{worktree_path}/docs/Agent/feature/README.md",
...
```bash
grep -oE '\[([^\]]+)\]\(([^)]+)\)' docs/Agent/feature/{feature}.md
```
...
File: docs/Agent/feature/{feature-name}.md
Sections populated:
...
3. Create new file: `docs/Agent/feature/opencode-backend-support.md`
...
File: docs/Agent/feature/opencode-backend-support.md
Sections populated:
...
**Scope:** `docs/Agent/feature/*.md` only

**Template:** Follow `docs/Agent/feature/template.md` structure

**Philosophy:** ~100 LOC per phase, smooth is safe, safe is fast

**Always:** Update README.md index, validate links

**References:** `docs/Agent/feature/template.md`, `docs/Agent/documentation_guide.md`
