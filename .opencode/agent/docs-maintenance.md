---
description: >-
  Subagent that manages maintenance documentation in docs/Agent/maintenance/.
  Invoked by the documentation primary agent to create and update maintenance
  docs for deprecations, migrations, release notes, and technical debt.
  
  This subagent:
  - Loads workflow context from adw_spec tool
  - Creates maintenance docs for deprecations, migrations
  - Updates existing maintenance guides
  - Creates release notes when appropriate
  - Archives old/superseded maintenance docs to archive/
  - Follows template.md format
  - Maintains docs/Agent/maintenance/README.md index
  - Validates markdown links
  
  Write permissions:
  - docs/Agent/maintenance/*.md: ALLOW
  - docs/Agent/maintenance/archive/*.md: ALLOW
mode: subagent
tools:
  adw_spec: true
  read: true
...
Create and update maintenance documentation in docs/Agent/maintenance/ for deprecations, migrations, release notes, and technical debt.
...
Update docs/Agent/maintenance/ if needed (migrations, release notes, etc.)
```
...
- @docs/Agent/maintenance/template.md - Maintenance doc template
- @docs/Agent/maintenance/README.md - Maintenance docs index
- @docs/Agent/documentation_guide.md - Documentation standards

# Write Permissions

**ALLOWED:**
- ✅ `docs/Agent/maintenance/*.md` - Create and update maintenance docs
- ✅ `docs/Agent/maintenance/archive/*.md` - Archive old/superseded docs
...
todowrite({
  "todos": [
    ...
    {
      "id": "3",
      "content": "Update docs/Agent/maintenance/README.md index",
      "status": "pending",
      "priority": "medium"
    },
    {
...
```python
read({"filePath": "{worktree_path}/docs/Agent/maintenance/template.md"})
```
...
```python
write({
  "filePath": "{worktree_path}/docs/Agent/maintenance/{feature}_migration_guide.md",
...
```python
write({
  "filePath": "{worktree_path}/docs/Agent/maintenance/release-notes-{version}.md",
...
```python
write({
  "filePath": "{worktree_path}/docs/Agent/maintenance/{feature}_deprecation.md",
...
```bash
# Move to archive
mv docs/Agent/maintenance/{old-doc}.md docs/Agent/maintenance/archive/
```
...
Read current index:
```python
read({"filePath": "{worktree_path}/docs/Agent/maintenance/README.md"})
```

Add new doc to appropriate section:
```python
edit({
  "filePath": "{worktree_path}/docs/Agent/maintenance/README.md",
...
```bash
grep -oE '\[([^\]]+)\]\(([^)]+)\)' docs/Agent/maintenance/{doc}.md
```
...
Files:
- docs/Agent/maintenance/{doc-name}.md ({new/updated})
...
Files:
- docs/Agent/maintenance/git_github_migration_guide.md (new)
...
**Scope:** `docs/Agent/maintenance/*.md` only

**Doc Types:** Migration Guide, Release Notes, Deprecation Notice, Technical Debt

**Always:** Update README.md index, validate links

**Template:** Follow `docs/Agent/maintenance/template.md` for technical debt tracking

**References:** `docs/Agent/maintenance/template.md`, `docs/Agent/documentation_guide.md`
