---
description: 'Use this agent to execute implementation plans by converting plan steps
  into todos and implementing them systematically. This agent operates in fully automated
  non-interactive mode.

  The agent will: - Read plan from spec_content via adw_spec tool - Move to isolated
  worktree from worktree_path - Convert plan steps to todo list automatically - Execute
  tasks in parallel where possible - Call tester agent to validate implementation
  - Call linter agent to validate code quality - Call adw-commit agent to commit changes
  - Retry up to 3 times on failure - Operate with no user input (autonomous decision-making)
...
Use the `worktree_path` for all operations and validate context with tools (no bash):
...
- `list({"path": worktree_path})` if directory validation is needed
...
**Verification:**
- Ensure branch matches the feature branch from git_operations output
- Confirm repository files are visible via list tool (no bash `ls`)
...
After linting passes, call adw-commit subagent to create properly formatted commit:
...
**What adw-commit subagent does:**
- Loads workflow context from state
- Uses `git_operations` for status/diff/add/commit to create the commit
- No push operations occur in this subagent (push is denied/omitted)
- Can actively fix pre-commit hook failures using `run_linters` with auto-fix
- Handles pre-commit hook failures (up to 3 retries with self-healing)
- Reports commit hash or failure

**Parse adw-commit output:**
- Success: Contains "ADW_COMMIT_SUCCESS" with commit hash
- Failure: Contains "ADW_COMMIT_FAILED" with error details
- Skip: Contains "ADW_COMMIT_SKIPPED" (no changes - not an error)

**On commit failure:**
- If attempt < 3: Retry adw-commit (pre-commit hooks may need fixes)
...
- Use `worktree_path` from state for all file operations
- Validate context with `git_operations({"command": "status", "worktree_path": worktree_path})`
