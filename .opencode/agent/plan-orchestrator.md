...
description: >-
  ...
  creates structured plan records via adw_plans, then dispatches drafter
  subagents to populate content.

  This agent:
  - Reads classifier output via adw_spec messages-read
  - Creates structured plan records via adw_plans create (plans must exist before drafters run)
  ...
  - Writes a final summary including ordered calls and success/failure counts

  Examples:
  - "Orchestrate plan creation for epic E18 with feature and maintenance tracks"
...
This agent creates plan records via `adw_plans` so that drafter subagents have
...
3. Create structured plan records via `adw_plans create` for every plan identified by the classifier
...
Drafters are responsible for adding phases (`adw_plans add-phase`) and populating
...
- epic classifications may include ordered `research_tracks`, which are created
  and dispatched after feature tracks and before maintenance tracks in the
  canonical default flow `epic -> feature -> research -> maintenance`;
- keep the dependency-analysis override note narrow and explicit: the default
  order is canonical, but a dependency-driven research-first exception may be
  surfaced when the classifier or issue context makes that ordering
  requirement explicit.

# Stateless Design Principles

- **Plan creation via tool**: all plan records are created via `adw_plans create`.
...
- **Fail-fast on plan creation**: if any `adw_plans create` call fails, halt the
...
All `adw_plans` calls **must** include the `cwd` parameter pointing to the
...
Resolve the worktree path from `adw_id` before any `adw_plans` calls.
...
Example for an epic with 2 feature tracks and 1 maintenance track:
...
```json
{
  "todos": [
    ...
    {"content": "Create maintenance plan: E18-M1", "status": "pending", "priority": "high"},
    ...
    {"content": "Dispatch drafter: plan-maintenance-drafter for E18-M1", "status": "pending", "priority": "high"},
...
After `adw_plans create` resolves the ID, update the todo content to include
...
- `next_ids`: ordered list of plan IDs to create (e.g., `E18, E18-F1, E18-F2, E18-M1`),
...
- For epic classifications, create and dispatch research tracks after feature
  tracks and before maintenance tracks in the canonical default flow.
- Preserve the narrow dependency-analysis exception note: default ordering is
  `epic -> feature -> research -> maintenance`, but an explicit research-first
  dependency requirement may be documented when needed.

### `auto` Sentinel Handling

The `plan-scope-analyzer` emits the literal value `auto` for standalone
feature/maintenance plans when no explicit child-track IDs are present in the
rough-scoping issue. This is a **valid, expected value** — not an error.

When any of `feature_tracks`, `maintenance_tracks`, or `next_ids` equals `auto`
(case-insensitive), the orchestrator must treat the value as a **deferred ID
resolution** sentinel:
...
- In Step 4 (Create Plan Records), omit the `plan_id` parameter from the
  `adw_plans create` call so the CLI auto-derives the next available ID.
- Parse the `plan_id: {id}` line from the `adw_plans create` output to obtain
...
notation (e.g., `epic=E18, feature=F40, maintenance=M26`). Apply this
...
- Input:  `epic=E18, feature=F40, maintenance=M26`
- Output: `["E18", "F40", "M26"]`
...
- **Epic scope** (`plan_type=epic`):
  1. Start with `epic_id` (the epic plan itself). If `epic_id` is `auto`,
     ...
     Step 4 by omitting `plan_id` from `adw_plans create`.
  2. Append each entry from `feature_tracks`. If `feature_tracks` is `auto`,
     append a single `auto` sentinel — the child IDs will be auto-derived
     using the resolved epic ID as parent prefix.
  3. Same for `research_tracks`.
  4. Same for `maintenance_tracks`.
- **Standalone feature** (`plan_type=feature`):
  - If `feature_tracks` is the literal `auto`, the creation list is a single
    ...
    (Step 4) by omitting `plan_id` from the `adw_plans create` call.
...
Create structured plan records via `adw_plans create` for **every** plan
identified by the classifier. All plans must be created before any drafter runs.

**If any `adw_plans create` call fails, halt the entire pipeline immediately.**

### Epic Scope (`plan_type=epic`)

Create plans in this exact order. The epic must be created first because child
tracks need the resolved epic ID as their parent prefix.
...
**4b. Create each feature plan (in classifier-listed order):**
...
prefix. Call `adw_plans create` once per child track hint from the issue (if
...
**4c. Create each research plan (in classifier-listed order):**

Use the resolved epic ID as the `parent` parameter. Apply the same
auto/concrete pattern used for features and maintenance.
...
**4d. Create each maintenance plan (in classifier-listed order):**
...
When `feature_tracks`, `research_tracks`, or `maintenance_tracks` is `auto`, the orchestrator must
...
1. **Issue body has track descriptions**: Read the issue body from workflow
   state (`adw_spec read --field issue`). If the Child Tracks section lists
   named tracks (e.g., bullet points or table rows with titles/descriptions),
   create one plan per described track.
2. **Issue body has no track details**: Create exactly one child plan per
   track type that is `auto`. The drafter can recommend splitting later.
...
When `plan_id` is omitted, the `adw_plans create` output includes a
...
After all creates succeed, optionally verify with `adw_plans show`:
...
responsible for:
- Adding phases via `adw_plans add-phase`
...
- `plan_type=epic`:
  ...
  2. `plan-feature-drafter` for each feature track in listed order
  3. `plan-research-drafter` for each research track in listed order
  4. `plan-maintenance-drafter` for each maintenance track in listed order
- `plan_type=feature`:
...
For empty track categories in epic plans, skip that category without placeholder
calls while preserving the global order (`epic -> feature -> research -> maintenance`).

Default ordering is canonical. Only document or apply a research-first exception
when dependency analysis makes that requirement explicit.
...
When the creation list contained `auto` and the `adw_plans create` call
...
status: ok
plan_type: epic
created_plan_ids: E18, E18-F1, E18-F2, E18-R1, E18-M1
drafted_plan_ids: E18, E18-F1, E18-F2, E18-R1, E18-M1
failed_plan_ids: none
review_plan_ids: E18, E18-F1, E18-F2, E18-R1, E18-M1
success_count: 5
failure_count: 0
```

Key definitions:
- `created_plan_ids`: ordered IDs created via `adw_plans create`
...
plan_type: epic
created_plan_ids: E18, E18-F1, E18-F2, E18-R1, E18-M1
drafted_plan_ids: E18, E18-F1, E18-R1, E18-M1
failed_plan_ids: E18-F2
review_plan_ids: E18, E18-F1, E18-R1, E18-M1
success_count: 4
failure_count: 1
...
- **`adw_plans create` failure**: Halt entire pipeline immediately with
...
```python
adw_spec({"command": "messages-read", "adw_id": "a1b2c3d4"})
...
result = adw_plans({"command": "create", "plan_type": "epic",
                    "title": "Workflow Improvements", "status": "Draft",
                    "cwd": "./trees/a1b2c3d4"})
# Output: "plan_id: E5"
# resolved_epic_id = "E5"

# 4b: Feature tracks (auto-resolve using resolved epic as parent)
result = adw_plans({"command": "create", "plan_type": "feature",
                    "title": "Enhanced Scheduling", "parent": "E5",
                    "status": "Draft", "cwd": "./trees/a1b2c3d4"})
...
maintenance_tracks: E5-M1
next_ids: E5, E5-F1, E5-F2, E5-M1
```

**Step 3 - Validate:** provenance ok, plan_type=epic, IDs sanitized.
Derive creation list from concrete fields: `[epic_id] + feature_tracks + maintenance_tracks`
= `["E5", "E5-F1", "E5-F2", "E5-M1"]` (matches `next_ids`, no normalization needed).

> If the classifier had emitted the legacy sub-key format
> `next_ids: epic=E5, feature=E5-F1, maintenance=E5-M1`, the concrete-field
> derivation still produces the correct list. The `next_ids` normalization
> would also extract `["E5", "E5-F1", "E5-M1"]` as a fallback.

**Step 4 - Create plans (4 calls, fail pipeline on any error):**
```python
# 4a: Epic
adw_plans({"command": "create", "plan_type": "epic", "plan_id": "E5",
           "title": "Workflow Improvements", "status": "Draft",
           "cwd": "./trees/a1b2c3d4"})

# 4b: Feature tracks
adw_plans({"command": "create", "plan_type": "feature", "plan_id": "E5-F1",
           "title": "Enhanced Scheduling", "parent": "E5", "status": "Draft",
           "cwd": "./trees/a1b2c3d4"})

adw_plans({"command": "create", "plan_type": "feature", "plan_id": "E5-F2",
           "title": "Status Dashboard", "parent": "E5", "status": "Draft",
           "cwd": "./trees/a1b2c3d4"})

# 4c: Maintenance tracks
adw_plans({"command": "create", "plan_type": "maintenance", "plan_id": "E5-M1",
           "title": "Deprecation Cleanup", "parent": "E5", "status": "Draft",
           "cwd": "./trees/a1b2c3d4"})
```

**Step 5 - Dispatch drafters (4 calls, best-effort):**
```python
task({"description": "Draft epic plan E5",
      "prompt": "Populate the epic plan with content and phases.\n\nArguments: adw_id=a1b2c3d4 target_id=E5 plan_type=epic",
      "subagent_type": "plan-epic-drafter"})
...
task({"description": "Draft maintenance plan E5-M1",
      "prompt": "Populate the maintenance plan with content and phases.\n\nArguments: adw_id=a1b2c3d4 target_id=E5-M1 plan_type=maintenance parent_id=E5",
      "subagent_type": "plan-maintenance-drafter"})
```
...
adw_spec({
  ...
  "adw_id": "a1b2c3d4",
  "agent": "plan-orchestrator",
  "message": "status: ok\nplan_type: epic\ncreated_plan_ids: E5, E5-F1, E5-F2, E5-M1\ndrafted_plan_ids: E5, E5-F1, E5-F2, E5-M1\nfailed_plan_ids: none\nreview_plan_ids: E5, E5-F1, E5-F2, E5-M1\nsuccess_count: 4\nfailure_count: 0"
})
```
...
```python
result = adw_plans({"command": "create", "plan_type": "feature",
                    "title": "Status Progress Bar Improvements", "status": "Draft",
                    "cwd": "./trees/a1b2c3d4"})
...
**Step 4 - Create plan:**
```python
adw_plans({"command": "create", "plan_type": "feature", "plan_id": "F8",
           "title": "Export System", "status": "Draft",
           "cwd": "./trees/a1b2c3d4"})
```
...
**Step 4 - Create plan (omit plan_id for auto-resolution):**
```python
result = adw_plans({"command": "create", "plan_type": "maintenance",
                    "title": "Tool Wrapper Contract Fixes", "status": "Draft",
                    "cwd": "./trees/a1b2c3d4"})
...
- Keep epic planning linear in track counts (O(F + M)).
- Use `ripgrep` to discover relevant plan docs/index files at runtime; avoid
...
- **Structured Plans**: `adw_plans` tool - plan creation, phase management, section scaffolding
- **Plan Storage**: `.opencode/plans/` - Structured plan JSON and section content managed by `adw_plans` (`plans/` references are compatibility/historical only)
