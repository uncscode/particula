...
Creates isolated ADW workspaces with the deterministic setup required before
any LLM/AI work begins, including:
- Fetching the GitHub issue
- Generating a branch name
- Creating a worktree
- Initializing ADW state

Example:
    $ .opencode/tool/create_workspace.py 123 --workflow-type patch
"""
...
def find_project_root() -> Path:
    """Locate the repository root by searching for pyproject.toml or .git.

    Walks up the directory tree until it finds one of the sentinel files and
    returns the corresponding directory. If no sentinel is found, the current
    working directory is returned as a fallback.

    Returns:
        Path: Path to the detected project root directory.
    """
    current = Path.cwd()
...
def create_workspace_cli(  # noqa: C901
    ...
    """Create an ADW workspace via the CLI and format the response.

    Args:
        issue_number: GitHub issue number (e.g., "123")
        workflow_type: Workflow type (
            "complete", "patch", "document", "generate"
        )
    ...
    Returns:
        Tuple of (exit_code, output_string) where ``output_string`` is formatted
        according to ``output_mode`` (summary, full, or json).
    """
...
def main():
    """Parse CLI arguments and create a workspace for the requested issue.

    Returns:
        int: Exit code reported by :func:`create_workspace_cli` (0 for success).
    """
