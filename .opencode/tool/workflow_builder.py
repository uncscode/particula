...
Example:
    $ .opencode/tool/workflow_builder.py create --workflow-name example
...
def find_project_root() -> Path:
    ...
    Walks upward from the current working directory until it finds one of the
    sentinel files. If neither file exists, the current working directory is
...
def workflow_builder_tool(  # noqa: C901
    ...
    """Execute workflow builder commands and format the response for CLI usage.
    ...
    try:
        ...
        if command == "create":
            ...
            return (
                1,
                (
                    f"ERROR: Unknown command '{command}'. Valid commands: "
                    "create, add_step, remove_step, get, list, update, validate"
                ),
            )
...
def main():
    """Parse CLI arguments, run the workflow builder tool, and print the result.

    Returns:
        int: Exit code returned by :func:`workflow_builder_tool`.
    """
