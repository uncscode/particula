...
Examples:
  # Validation
  python3 .opencode/tools/validate_notebook.py docs/Examples --recursive --output json
  # Conversion
  python3 .opencode/tools/validate_notebook.py notebook.ipynb --convert-to-py
   python3 .opencode/tools/validate_notebook.py docs/Examples \\
       --recursive --convert-to-py --output-dir scripts
  python3 .opencode/tools/validate_notebook.py script.py --convert-to-ipynb
  # Sync
  python3 .opencode/tools/validate_notebook.py notebook.ipynb --sync
  # Check-sync (read-only)
  python3 .opencode/tools/validate_notebook.py docs/Examples --recursive --check-sync
"""
...
def _resolve_skip_syntax(args: argparse.Namespace) -> bool:
    ...
    if explicit_full_mode and args.skip_syntax:
        raise ValidationToolError(
            "--skip-syntax cannot be combined with --validation-mode full or --full"
        )
    ...
    if effective_mode == "fast":
        return True
    return bool(args.skip_syntax)


def _ensure_validation_only_flags(args: argparse.Namespace) -> None:
    convert_to_py = getattr(args, "convert_to_py", False)
    convert_to_ipynb = getattr(args, "convert_to_ipynb", False)
    validation_only_requested = any(
        [
            args.output != "summary",
            args.skip_syntax,
            getattr(args, "validation_mode", None) is not None,
            getattr(args, "fast", False),
            getattr(args, "full", False),
        ]
    )
    if validation_only_requested and (
        convert_to_py or convert_to_ipynb or args.sync or args.check_sync
    ):
        raise ValidationToolError(
            "--output/--skip-syntax/--validation-mode/--fast/--full can only be used with "
            "validation (omit convert/sync flags)"
        )
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  # Validation\n"
            "  python3 .opencode/tools/validate_notebook.py notebook.ipynb\n"
            ...
            "  python3 .opencode/tools/validate_notebook.py notebook.ipynb --sync\n"
            "  python3 .opencode/tools/validate_notebook.py"
            " docs/Examples --recursive --check-sync\n"
        ),
    )
    ...
    parser.add_argument(
        ...
        choices=["fast", "full"],
        help=(
            "Validation profile (validation only): 'fast' implies --skip-syntax, "
            "'full' enforces syntax checks"
        ),
    )
    parser.add_argument(
        "--fast",
        action="store_true",
        help="Alias for --validation-mode fast (validation only)",
    )
    parser.add_argument(
        "--full",
        action="store_true",
        help="Alias for --validation-mode full (validation only)",
    )
