...
Examples:
    ...
    python3 .opencode/tools/run_cmake.py --preset ninja-release

    # Configure with Ninja generator
    python3 .opencode/tools/run_cmake.py --ninja --source-dir example_cpp_dev

    # Full output mode for debugging
    python3 .opencode/tools/run_cmake.py --preset debug --output full
"""
...
def _bounded_append(collection: List[str], value: str, limit: int) -> None:
    """Append a value to a list without exceeding the collection limit.

    Args:
        collection: Mutable list receiving captured messages.
        value: Message string to append.
        limit: Maximum number of items allowed in ``collection``.

    Returns:
        None. The function mutates ``collection`` in place when space remains.
    """
...
def _decode_capture(value: bytes | str | None) -> str:
    """Normalize subprocess-captured output to a safe text string.

    This helper accepts subprocess capture values that may be ``bytes``, ``str``,
    or ``None`` and returns a consistent string representation. Byte payloads are
    decoded as UTF-8 with replacement to avoid decode-time failures.

    Args:
        value: Captured ``stdout``/``stderr`` value from ``subprocess.run``.

    Returns:
        Decoded text content, or an empty string when ``value`` is ``None``.
    """
    if value is None:
        return ""
...
def build_cmake(
    ...
    are handled safely before parsing and formatting.
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  python3 .opencode/tools/run_cmake.py --preset debug\n"
            "  python3 .opencode/tools/run_cmake.py --ninja --source-dir example_cpp_dev\n"
