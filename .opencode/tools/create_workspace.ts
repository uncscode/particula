...
export default tool({
  ...
  Sparse-call rule: omit optional fields unless intentionally set. Blank optional strings such as adwId or triggeredBy are treated as omitted.`,
  ...
  args: {
    ...
    adwId: tool.schema
      .string()
      .optional()
      .describe("Optional existing ADW ID (8-char hex like 'abc12345'). If omitted or blank, generates new one."),
  ...
  async execute(args) {
    ...
    const triggeredBy = normalizeOptionalString(args.triggeredBy) || "manual";
    ...
    // Add optional adw-id if provided — validate via canonical normalizeAdwId
    const adwId = normalizeOptionalString(args.adwId);
    if (adwId !== undefined) {
      const normalizedAdwId = normalizeAdwId(adwId);
      if (!normalizedAdwId) {
