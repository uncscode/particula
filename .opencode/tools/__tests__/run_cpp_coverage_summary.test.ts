...
describe("run_cpp_coverage_summary wrapper", () => {
  ...
  it("builds routine coverage command from bounded options", async () => {
    ...
    const result = await execute({ buildDir: ".", threshold: 80, timeout: 60, options: "output=full" });
  ...
  it("rejects legacy direct fields and advanced keys by presence including falsey payloads", async () => {
    ...
    expect(await execute({ buildDir: ".", tool: "" })).toContain("does not accept advanced option 'tool'");
    expect(getInvocations()).toHaveLength(0);
  });

  it("fails closed for invalid options, threshold validation, and path validation", async () => {
    const execute = await loadToolExecute("../../run_cpp_coverage_summary.ts");
    expect(await execute({ buildDir: ".", options: "output=xml" })).toContain("output must be one of summary, full, json");
    expect(await execute({ buildDir: ".", threshold: 101 })).toContain("threshold must be between 0 and 100");
