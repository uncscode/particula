...
describe("run_cpp_coverage_advanced wrapper", () => {
  ...
  it("builds advanced command with bounded tool and direct filter/html", async () => {
    ...
    const result = await execute({ buildDir: ".", filter: "src/", html: ".", options: "output=json tool=llvm-cov" });
    ...
    expect(cmd).toContain(`--html=${realpathSync(".")}`);
  });

  it("omits blank optional strings and rejects legacy direct fields or invalid option tokens", async () => {
    const execute = await loadToolExecute("../../run_cpp_coverage_advanced.ts");
    await execute({ buildDir: ".", filter: " ", html: " ", options: "output=summary" });
    const cmd = getInvocations().at(-1)?.args.join(" ") ?? "";
    expect(cmd).not.toContain("--filter=");
    expect(cmd).not.toContain("--html=");

    resetSubprocessMocks();
    expect(await execute({ buildDir: ".", outputMode: "summary" })).toContain("does not accept direct field 'outputMode'");
    expect(await execute({ buildDir: ".", tool: "gcov" })).toContain("does not accept direct field 'tool'");
    expect(await execute({ buildDir: ".", extraArgs: ["--flag"] })).toContain("does not accept direct field 'extraArgs'");
    expect(await execute({ buildDir: ".", options: "tool=bad" })).toContain("tool must be one of gcov, llvm-cov");
    expect(getInvocations()).toHaveLength(0);
  });

  it("fails closed for html, threshold, and buildDir path validation", async () => {
    const execute = await loadToolExecute("../../run_cpp_coverage_advanced.ts");
    expect(await execute({ buildDir: ".", html: import.meta.path })).toContain("html path is not a directory");
    expect(await execute({ buildDir: ".", threshold: -1 })).toContain("threshold must be between 0 and 100");
  ...
  it("canonicalizes and allows safe nested new html output directories", async () => {
    const execute = await loadToolExecute("../../run_cpp_coverage_advanced.ts");
    const parentDir = path.join("tmp_cpp_wrapper_html_parent");
    mkdirSync(parentDir, { recursive: true });
    const nestedHtmlDir = path.join(parentDir, "nested", "coverage_html");

    const result = await execute({ buildDir: ".", html: nestedHtmlDir, options: "output=summary" });
    expect(result).toBe("ok");
  ...
  it("preserves failure diagnostics and missing-script hinting", async () => {
    ...
    const result = await execute({ buildDir: "." });
  ...
  it("prefers stdout over stderr on subprocess failure", async () => {
    ...
    expect(await execute({ buildDir: "." })).toBe("stdout diagnostic");
  });
});
