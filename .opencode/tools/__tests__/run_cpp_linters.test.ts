...
describe("run_cpp_linters wrapper", () => {
  ...
  it("preserves explicit autoFix bridge with bounded options", async () => {
    ...
    const result = await execute({ sourceDir: ".", autoFix: true, options: "output=json linters=clang-tidy" });
  ...
  it("omits auto-fix when falsey and rejects legacy direct fields or invalid options", async () => {
    const execute = await loadToolExecute("../../run_cpp_linters.ts");
    await execute({ sourceDir: ".", options: "linters=clang-format" });
    expect(getInvocations().at(-1)?.args.join(" ") ?? "").not.toContain("--auto-fix");

    resetSubprocessMocks();
    expect(await execute({ sourceDir: ".", outputMode: "json" })).toContain("does not accept direct field 'outputMode'");
    expect(await execute({ sourceDir: ".", linters: ["clang-tidy"] })).toContain("does not accept direct field 'linters'");
    expect(await execute({ sourceDir: ".", options: "output=xml" })).toContain("output must be one of summary, full, json");
