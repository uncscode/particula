...
describe("platform_pr_read wrapper", () => {
  ...
  it("assembles pr-comments", async () => {
    ...
    expect(getInvocations().at(-1)?.args.join(" ")).toContain("uv run adw platform pr-comments 12");
  });
  ...
  it("accepts leading-zero issue_number and assembles prefer_scope/actionable_only", async () => {
    ...
    expect(getInvocations().at(-1)?.args.join(" ")).toContain(
      "uv run adw platform pr-comments 00012 --actionable-only --prefer-scope fork",
    );
  });
  ...
  it("assembles pr-diff with format and prefer_scope", async () => {
    ...
    expect(getInvocations().at(-1)?.args.join(" ")).toContain(
      "uv run adw platform pr-diff 12 --format json --prefer-scope upstream",
    );
  });
  ...
  it("preserves delegated structured json failure payloads for pr-diff json mode", async () => {
    const execute = await loadToolExecute("../../platform_pr_read.ts");
    setDollarError(
      buildDollarFailure({
        stdout: JSON.stringify({ success: false, error: "diff fetch failed", scope: "upstream" }),
