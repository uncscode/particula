...
Usage:
    python3 .opencode/tools/run_cpp_coverage.py --build-dir build
    python3 .opencode/tools/run_cpp_coverage.py --build-dir build --threshold 80
    python3 .opencode/tools/run_cpp_coverage.py --build-dir build --html coverage_html --output json

Examples:
    # Generate coverage report
    python3 .opencode/tools/run_cpp_coverage.py --build-dir example_cpp_dev/build/coverage

    # Validate 80% line coverage threshold
    python3 .opencode/tools/run_cpp_coverage.py --build-dir build --threshold 80

    # Generate HTML report
    python3 .opencode/tools/run_cpp_coverage.py --build-dir build --html coverage_html

    # Filter to specific directory
    python3 .opencode/tools/run_cpp_coverage.py --build-dir build --filter src/ --output json
"""
...
def format_summary(
    ...
    lines.append(
        f"  Branches:  {metrics.branches_covered}/{metrics.branches_total} ({branch_pct})"
    )
    func_pct = f"{metrics.function_percent:.1f}%"
    lines.append(
        f"  Functions: {metrics.functions_covered}/{metrics.functions_total} ({func_pct})"
    )
