...
- Reports file count and size before deletion`,
  ...
  async execute(args) {
    ...
    if (dryRun && force) {
      return "ERROR: clear_build mode conflict: choose either dryRun preview or force delete, not both.";
    }
    if (!dryRun && !force) {
      return "ERROR: clear_build destructive mode requires force: true when dryRun is false. Use dryRun: true to preview.";
    }
