...
export default tool({
  description: "Execute adw platform comment with strict validation.",
  args: {
    command: tool.schema.enum(["comment"]).describe("Command to execute."),
  ...
  async execute(args) {
    ...
    try {
      ...
      const stdout = sanitizeAndTruncate(error.stdout);
      const stderr = sanitizeAndTruncate(error.stderr);
      const parts: string[] = ["ERROR: Failed to execute 'adw platform comment'"];
      if (stderr) {
        parts.push(`STDERR:\n${stderr}`);
      }
      if (stdout) {
