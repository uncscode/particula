...
- Format-only check: run_cpp_lint_check({ sourceDir: 'src', options: 'linters=clang-format' })
- clang-tidy check: run_cpp_lint_check({ sourceDir: 'src', buildDir: 'build', options: 'linters=clang-tidy output=full' })
...
- clang-tidy requires compile_commands.json in buildDir`,
  ...
  async execute(args) {
    const legacyFieldError = rejectLegacyDirectFields(args, "run_cpp_lint_check", ["outputMode", "linters"]);
    if (legacyFieldError) {
    ...
    try {
      ...
      if (stderr.trim()) {
        ...
        return `ERROR: C++ lint check failed\n\n${stderr}${hint}`;
      }
