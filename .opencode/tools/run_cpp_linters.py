...
Usage:
    python3 .opencode/tools/run_cpp_linters.py --source-dir example_cpp_dev
    python3 .opencode/tools/run_cpp_linters.py --source-dir src --auto-fix
    python3 .opencode/tools/run_cpp_linters.py --source-dir src --linters clang-format
    python3 .opencode/tools/run_cpp_linters.py --source-dir src \\
        --build-dir build --linters clang-tidy
    python3 .opencode/tools/run_cpp_linters.py --source-dir src --output json
"""
...
class LinterResult:
    ...
    Attributes:
        name: Linter identifier (e.g. ``clang-format``).
        exit_code: Process exit code from the linter invocation.
        stdout: Captured standard output (possibly truncated).
        stderr: Captured standard error (possibly truncated).
        errors: Count of error-level diagnostics found.
        warnings: Count of warning-level diagnostics found.
        files_checked: Number of files passed to the linter.
        files_with_issues: Number of files that had at least one issue.
        success: Whether the linter run passed without errors.
        skipped: Whether the linter was skipped (e.g. not installed).
        error_message: Human-readable message when the run could not complete.
        issues: Bounded list of individual issue strings.
        duration: Wall-clock seconds for this linter invocation.
        truncated: Whether output was truncated to fit size limits.
        timeout: Whether the subprocess exceeded its timeout.
    """
...
def _bounded_append(collection: List[str], value: str, limit: int = ISSUE_CAPTURE_LIMIT) -> None:
    """Append a value to a list only when the list is below the size limit.

    Args:
        collection: Target list to append to.
        value: String value to append.
        limit: Maximum number of entries allowed in *collection*.
    """
...
def check_linter_available(linter: str) -> bool:
    """Check if a linter command is available on PATH.

    Args:
        linter: Executable name to look up (e.g. ``clang-format``).

    Returns:
        True when the executable is found on the system PATH.
    """
...
def run_clang_tidy(
    ...
    for start in range(0, len(files), CLANG_TIDY_BATCH_SIZE):
        ...
        cmd = ["clang-tidy", "--allow-no-checks", f"-p={build_dir}"]
        if auto_fix:
            cmd.append("--fix")
...
def format_full_output(results: List[LinterResult], summary: str) -> str:
    ...
    Args:
        results: Linter results whose raw output should be included.
        summary: Pre-formatted summary block appended at the end.

    Returns:
        Multi-line string containing per-linter output sections followed
        by the summary.
    """
...
def format_json_output(results: List[LinterResult], duration: float, all_skipped: bool) -> str:
    ...
    Args:
        results: Linter results to serialize.
        duration: Total wall-clock seconds across all linter runs.
        all_skipped: Whether every linter was skipped.

    Returns:
        Pretty-printed JSON string containing per-linter detail and an
