...
SPARSE OPTIONALS: Only include optional fields when meaningful. Blank optional cwd/writeExecuted values are treated as omitted; timeout must be positive when provided.`,
  args: {
    ...
    timeout: tool.schema
      .number()
      .optional()
      .describe("Timeout in seconds per notebook (default: 600). Must be positive when provided."),
    ...
    cwd: tool.schema
      .string()
      .optional()
      .describe("Working directory for execution (default: project root). Blank strings are treated as omitted."),
    writeExecuted: tool.schema
      .string()
      .optional()
      .describe(
        "Directory where executed notebook copies should be written; when overwriting (default) source is also updated. Blank strings are treated as omitted.",
      ),
