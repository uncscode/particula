...
// --- Inlined preview command builder from refactor_astgrep_preview.ts ---
...
// --- Inlined apply command builder from refactor_astgrep_apply.ts ---

const buildApplyCommand = (args: RefactorAstgrepNormalizedArgs): string[] => {
  return ["ast-grep", "run", "-p", args.pattern, "-r", args.rewrite, "-l", args.lang, "--update-all", "--", args.path];
};

// --- Inlined execute logic from preview and apply tools ---

async function executePreview(args: Record<string, unknown>): Promise<string> {
  const normalized = normalizeAndValidateArgs(args);
...
async function executeApply(args: Record<string, unknown>): Promise<string> {
  const normalized = normalizeAndValidateArgs(args);
  if (typeof normalized === "string") {
    return normalized;
  }

  const cmd = buildApplyCommand(normalized);
  try {
    const result = await Bun.$`${cmd}`.text();
    if (result.trim()) {
      return result;
    ...
    return formatExecutionError(error, "Failed to execute 'ast-grep apply'.", "apply");
  }
}

// --- Tool definition ---

const DESCRIPTION = `Compatibility wrapper for ast-grep refactors.

This wrapper delegates to split wrappers:
- refactor_astgrep_preview (non-mutating)
- refactor_astgrep_apply (mutating)

By default this wrapper behaves like preview mode (dry-run).

Refactor code using ast-grep (AST-aware find-and-rewrite) with dry-run by default.
...
SPARSE OPTIONALS: Omit optional fields unless meaningful. Blank path values are treated as omitted and default to '.'.`;
...
export default tool({
  ...
  args: {
    ...
    path: tool.schema
      .string()
      .optional()
      .describe("File or directory to search (default: '.'). Blank strings are treated as omitted."),
    dryRun: tool.schema
      .boolean()
      .optional()
  ...
  async execute(args) {
    ...
    if (dryRun) {
      return executePreview(args as Record<string, unknown>);
    }
    return executeApply(args as Record<string, unknown>);
  },
});
