...
# Get organic density using the current API
...
# New API returns kg/m^3 (SI units), convert to g/cm^3 for compatibility
density_organics_kg_m3 = get_organic_density_array(
    molar_mass=M_gmol,
    oxygen2carbon=OC_ratio,
    hydrogen2carbon=HC_ratio,
)  # kg/m^3 (SI units)
density_organics_g_cm3 = density_organics_kg_m3 / 1000  # Convert to g/cm^3
...
particle_representation = (
    ...
    .set_charge(
        np.zeros_like(number_concentration)
    )  # Charge array matching bins
    .set_density(particle_densities, "kg/m^3")
