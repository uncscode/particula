...
-   __[Data Containers](Data_Containers/index.md)__
    ...
    - [GPU-resident graph-capture source](https://github.com/Gorkowski/particula/blob/main/docs/Examples/gpu_resident_graph_capture.py) — a lazy native-CUDA-only walkthrough that explicitly captures, replays twice, retires, and renews one fixed resident binding. Run `python docs/Examples/gpu_resident_graph_capture.py`; set `PARTICULA_EXAMPLE_FORCE_NO_NATIVE_CAPTURE=1` to verify that no CPU or Warp-CPU fallback runs.
