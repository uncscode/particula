...
def liquid_vapor_obj_function(
    ...
    denominator_alpha = [1 / (1 - mass_factions)
                         if mass_factions < 1 else 0
                         for mass_factions in mass_fraction_water_ab[:, 0]]
    c_j_aq_alpha = np.sum(
        c_j_alpha
        * mass_fraction_water_ab[:, 0]
        * denominator_alpha)  # type: ignore
    ...
    mass_weighted_molar_mass_alpha = np.sum(
        c_j_alpha / molar_mass) + c_j_aq_alpha / 18.015
    mass_weighted_molar_mass_beta = np.sum(
        c_j_beta / molar_mass) + c_j_aq_beta / 18.015
    ...
    if mass_weighted_molar_mass_alpha > 0:
        c_star_j_via_alpha = c_star_j_dry * gamma_organic_ab[:, 0] \
            * q_ab[:, 0] \
            * c_liquid_total / (molar_mass * mass_weighted_molar_mass_alpha)
    else:
        c_star_j_via_alpha = np.zeros_like(c_star_j_dry)

    if mass_weighted_molar_mass_beta > 0:
        c_star_j_via_beta = c_star_j_dry * gamma_organic_ab[:, 1] \
            * q_ab[:, 1] \
            * c_liquid_total / (molar_mass * mass_weighted_molar_mass_beta)
    else:
    ...
    c_star_j_new = c_star_j_via_alpha * q_ab[:, 0] \
    ...
    # guess/new
    error_out = np.sum(((e_j_partition_guess - e_j_partition_new)**2
    ...
    c_j_aq_new_alpha = c_j_liquid_new_alpha * mass_fraction_water_ab[:, 0] \
        * denominator_alpha
    ...
    c_j_aq_new_beta = c_j_liquid_new_beta * mass_fraction_water_ab[:, 1] \
        * denominator_beta
    ...
    alpha_phase_output = (c_j_liquid_new_alpha, c_j_aq_new_alpha,
                          c_liquid_new_alpha, c_aq_new_alpha)
    beta_phase_output = (c_j_liquid_new_beta, c_j_aq_new_beta,
                         c_liquid_new_beta, c_aq_new_beta)
    system_output = (c_liquid_total_new, c_liquid_total_water_new,
