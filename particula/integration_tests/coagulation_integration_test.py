...
class TestCoagulationIntegration(unittest.TestCase):
    ...
    def test_pmf_initial_distribution(self):
        ...
        self.assertEqual(aerosol_pmf.particles[0].get_concentration().shape, self.radius_bins.shape)
    ...
    def test_pdf_initial_distribution(self):
        ...
        self.assertEqual(aerosol_pdf.particles[0].get_concentration().shape, self.radius_bins.shape)
    ...
    def test_resolved_initial_distribution(self):
        ...
        particle_mass_sample = (
            4 / 3 * np.pi * radii_sample**3 * self.density
        )
        resolved_masses = (
        ...
        self.assertAlmostEqual(np.sum(particle_mass_sample), aerosol_resolved.particles[0].get_mass_concentration(), delta=1e-6)
    ...
    def test_coagulation_process(self):
        ...
        self.assertAlmostEqual(initial_mass_pmf, aerosol_pmf.particles[0].get_mass_concentration(), delta=1e-6)
        self.assertAlmostEqual(initial_mass_pdf, aerosol_pdf.particles[0].get_mass_concentration(), delta=1e-6)
        self.assertAlmostEqual(initial_mass_resolved, aerosol_resolved.particles[0].get_mass_concentration(), delta=1e-6)
