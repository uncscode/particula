...
def test_coerce_type():
    ...
    assert np.array_equal(
        convert.coerce_type([1, 2, 3], np.ndarray), np.array([1, 2, 3])
    )
...
def test_length_to_volume():
    ...
    # Test with radius
    assert np.isclose(convert.length_to_volume(3), 113.0973, rtol=1e-4)

    # Test with diameter
    assert np.isclose(
        convert.length_to_volume(6, "diameter"), 113.09733552923254, rtol=1e-4
    )
...
def test_mole_fraction_to_mass_fraction():
    ...
    assert convert.mole_fraction_to_mass_fraction(
        mole_fraction0=0.5, molecular_weight0=50, molecular_weight1=100
...
def test_mole_fraction_to_mass_fraction_multi():
    ...
    assert np.allclose(
        convert.mole_fraction_to_mass_fraction_multi(
            mole_fractions=[0.25, 0.25, 0.5], molecular_weights=[200, 200, 100]
        ),
...
def test_mass_fraction_to_volume_fraction():
    ...
    assert convert.mass_fraction_to_volume_fraction(0.5, 1.5, 2) == (
        0.5714285714285715,
        0.4285714285714285,
    )
...
def test_volume_water_from_volume_fraction():
    ...
    assert (
        convert.volume_water_from_volume_fraction(
            volume_solute_dry=100, volume_fraction_water=0.8
        )
        == 400.0000000000001
    )
