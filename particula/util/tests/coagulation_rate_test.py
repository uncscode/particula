...
def test_mass():
    ...
    assert np.trapz((gain - loss)*rads**3, rads.m_as(u.m)).u == u.s**-1
    assert np.trapz((gain - loss)*rads**3, rads.m_as(u.m)) <= 0.2 * u.s**-1
    assert np.trapz(
        (gain - loss).to(u.cm**-3/u.s)*rads**3, rads.m_as(u.m)
    ).m == pytest.approx(0.0)
