...
def get_pairwise_min_matrix(
    input_array: NDArray[np.float64],
) -> NDArray[np.float64]:
    """
    Compute the pairwise minimum matrix for a given 1D NumPy array.

    This function performs a self-broadcasting operation to generate a matrix
    where each element (i, j) is computed as:

        output_matrix[i, j] = min(input_array[i], input_array[j])
    ...
    Returns:
    --------
        - A 2D NumPy array where each element is the minimum of two
          elements from the input array.
    ...
    ``` py title="Example"
    input_array = np.array([1, 2, 3])
    get_pairwise_min_matrix(input_array)
    array([[1, 1, 1],
           [1, 2, 2],
           [1, 2, 3]])
    ```
    """
    return np.minimum(input_array[:, np.newaxis], input_array[np.newaxis, :])
