...
from particula.aerosol_dynamics import Particle, Environment


def test_getters():
    """ test the getters of the Particle class.
    """

    default_particle = Particle(radius=1e-9)
    named_particle = Particle(
        radius=1 * u.nm,
        density=1.7 * u.g / u.cm**3,
        charge=1,
        name="test",
    )

    assert default_particle.radius() == 1e-9 * u.m
    assert default_particle.mass().units == u.kg
    assert default_particle.charge() == 0
    assert default_particle.density().magnitude == 1000

    assert named_particle.radius() == 1e-9 * u.m
    assert named_particle.mass().units == u.kg
...
def test_methods():
    ...
    assert (
        small_particle.knudsen_number() ==
        pytest.approx(66.4, rel=1e-1)
    )
    assert (
