...
def ratio(
    ...
    numerator = -1 * charge * np.transpose(charge) * (
        ELEMENTARY_CHARGE_VALUE.m ** 2
    )
    denominator = 4 * np.pi * ELECTRIC_PERMITTIVITY.m * (
...
def kinetic(
    ...
    return np.where(coulomb_potential >= 0,
                    1 + coulomb_potential,
...
def continuum(
    ...
    # otherwise 1
    return np.where(coulomb_potential != 0,
                    coulomb_potential / (1 - np.exp(-1 * coulomb_potential)),
                    1)
