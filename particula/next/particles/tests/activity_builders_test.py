...
from particula.next.particles.activity_builders import (
    IdealActivityMolarBuilder, IdealActivityMassBuilder,
...
def test_build_ideal_activity_mass():
    ...
    builder = IdealActivityMassBuilder()
...
def test_build_ideal_activity_molar_parameter():
    ...
    builder = IdealActivityMolarBuilder()
...
def test_build_ideal_activity_molar_dict():
    ...
    builder_dict = IdealActivityMolarBuilder()
    parameters = {
        "molar_mass": 1,
...
def test_build_ideal_activity_molar_missing_parameters():
    ...
    builder_missing = IdealActivityMolarBuilder()
    with pytest.raises(ValueError) as excinfo:
...
def test_build_kappa_parameter_activity_set_kappa():
    ...
    builder = KappaParameterActivityBuilder()
    builder.set_kappa(1)
...
def test_build_kappa_parameter_activity_dict():
    ...
    builder_dict = KappaParameterActivityBuilder()
    parameters = {
    ...
    builder_missing2 = KappaParameterActivityBuilder()
    parameters = {
        "kappa": np.array([0, 1]),
    ...
    assert (
        "Missing required parameter(s): water_index" in str(excinfo.value))
