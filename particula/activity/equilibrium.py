...
def VBS_equilibration_withLLEpartition_objFun_KGv3(guess_Ej, C_OM_ugPm3, Cstar_dry, activity_coefficient_AB, q_alpha_molefrac_phase_split_org, mass_fraction_water_AB, molecular_weight):
    ...
    mass_fraction_water_alpha_denominator = abs(mass_fraction_water_alpha_denominator)*alpha_good_denominators + np.logical_not(alpha_good_denominators)
    mass_fraction_water_beta_denominator = abs(mass_fraction_water_beta_denominator)*beta_good_denominators + np.logical_not(beta_good_denominators)

    Coa_j = guess_Ej * C_OM_ugPm3
    Coa_guess_viaEj = np.sum(Coa_j)

    Coa_j_alpha = Coa_j * q_alpha
    Caq_alpha = np.sum(Coa_j_alpha*mass_fraction_water_alpha/mass_fraction_water_alpha_denominator)
    Coaaq_alpha = np.sum(Coa_j_alpha) + Caq_alpha

    Coa_j_beta = Coa_j * q_beta
    Caq_beta = np.sum(Coa_j_beta*mass_fraction_water_beta/mass_fraction_water_beta_denominator)
    Coaaq_beta = np.sum(Coa_j_beta) + Caq_beta

    Coaaq_via_Ej_guess = Coaaq_beta + Coaaq_alpha

    massweighted_molar_weight_alpha = np.sum(Coa_j_alpha/molecular_weight) + Caq_alpha/18.015

    if massweighted_molar_weight_alpha > 0:
        Cstar_j_via_alpha = Cstar_dry * activity_coefficient_alpha * q_alpha * Coaaq_via_Ej_guess / (molecular_weight * massweighted_molar_weight_alpha)
    else:
        Cstar_j_via_alpha = q_alpha * 0

    massweighted_molar_weight_beta = np.sum(Coa_j_beta/molecular_weight) + Caq_beta/18.015

    if massweighted_molar_weight_beta > 0:
        Cstar_j_via_beta = Cstar_dry * activity_coefficient_beta * q_beta * Coaaq_via_Ej_guess / (molecular_weight * massweighted_molar_weight_beta)
    else:
    ...
    error_out = np.sum((guess_Ej - Ej_new)**2 + (Coa_guess_viaEj - Coa_new_viaEj)**2)
...
def VBS_equilibration_withLLEpartition_KGv2(guess_C_OAalpha_ugPm3, guess_C_OAbeta_ugPm3, guess_partition_coefficients, C_OM_ugPm3, Cstar_dry, activity_coefficient_AB, q_alpha_molefrac_phase_split_org, mass_fraction_water_AB, molecular_weight, a_water, O2C_values, BAT_functional_group, VBSBAT_options):
    
    Molar_mass_ratios = 18.015 / molecular_weight
    if guess_C_OAbeta_ugPm3 > 0:
        guess_C_oaaqbeta_ugPm3 = guess_C_OAbeta_ugPm3 / (1 - np.mean(mass_fraction_water_AB[:,1]))
    elif guess_C_OAalpha_ugPm3 > 0:
        guess_C_oaaqbeta_ugPm3 = guess_C_OAalpha_ugPm3 / (1 - np.mean(mass_fraction_water_AB[:,1]))
    else:
        guess_C_oaaqbeta_ugPm3 = 1 / (1 - np.mean(mass_fraction_water_AB[:,1]))

    if np.sum(guess_partition_coefficients) == 0 or VBSBAT_options['optimization']['independent_aw'].lower() == 'yes':
        
        if VBSBAT_options['VBSBAT_NN_options']['use_NN_for_VBS_initial_guess'].lower() == 'no': # guess for Ej using approximation
            Cstar_j_guess = Cstar_dry * activity_coefficient_AB[:,1] / (1 + mass_fraction_water_AB[:,1] * (1 / Molar_mass_ratios - 1)) #not right but fine for guess
            Ej_guess = (1 + Cstar_j_guess / guess_C_oaaqbeta_ugPm3) ** -1

        elif VBSBAT_options['VBSBAT_NN_options']['use_NN_for_VBS_initial_guess'].lower() == 'yes': # guess for Ej using neural network
            ...
            # Ej_guess = VBSBAT_nerual_network_estimate_v1(Cstar_dry, C_OM_ugPm3, O2C_temp, 18.015 / Mratio_temp, mass_fraction_water_AB[:,1], a_water, VBSBAT_options['VBSBAT_NN_options'])
            pass
    elif VBSBAT_options['optimization']['independent_aw'].lower() == 'no': # use previous Ej_guess results
        Ej_guess = guess_partition_coefficients
    else:
        raise ValueError(' Select VBSBAT_options.optimization.independent_aw option either yes or no')

    # error in guess, runs the cost funciton to see if there is an error in
    # guess.
    guess_error = VBS_equilibration_withLLEpartition_objFun_KGv3(Ej_guess, C_OM_ugPm3, Cstar_dry, activity_coefficient_AB, q_alpha_molefrac_phase_split_org, mass_fraction_water_AB, molecular_weight)[-1]

    if VBSBAT_options['optimization']['guess_refinement_threshold'] <= guess_error: # check if refinement is needed
        S_om = C_OM_ugPm3.shape
        bounds = Bounds(np.ones(S_om[0]) * 10**-10, np.ones(S_om[0])) # Coa qAlpha and Ej bounds
        ...
        if VBSBAT_options['optimization']['opt_method'].lower() == 'fmincon': # a single initialization point which is default
            fit_result = minimize(**problem)
            fit_CoaEj, fit_exit_flag = fit_result.x, fit_result.success
        elif VBSBAT_options['optimization']['opt_method'].lower() == 'global': # multiple start points taken at random
            ...
            raise ValueError('Invalid VBSBAT_options.optimization.opt_method... possible options fmincon, global, none')
    else: # guess has a lower error than guess_refinement_threshold
        fit_CoaEj = Ej_guess
        fit_exit_flag = np.NaN

    # runs a final calcuation for output
    partition_coefficients_AB, Coa_j_AB, Caq_j_AB, Cstar_j, Coa_AB, Caq_AB, Coa, q_alpha_water, error_out = VBS_equilibration_withLLEpartition_objFun_KGv3(fit_CoaEj, C_OM_ugPm3, Cstar_dry, activity_coefficient_AB, q_alpha_molefrac_phase_split_org, mass_fraction_water_AB, molecular_weight)
...
def VBS_equilibration_extractCsat_withLLEpartition_KGv2(Cp_j_VBSold, Cstar_j_VBSold, aw_measurment, molecular_weight, O2C_values, H2C_values, BAT_functional_group, BAT_refinement_mode, N2C_values_denistyOnly=None):
    ...
    _, mole_frac_org_beta = inverted_NNMcGlashan_v8(O2C_values,H2C_values, Molar_mass_ratios, aw_vec, BAT_functional_group)
    ...
    # beta 
    Coa_j_beta = Coa_j * q_beta
    Caq_beta = np.sum(Coa_j_beta * mass_fraction_water_beta / (1 - mass_fraction_water_beta))
    Coaaq_beta = np.sum(Coa_j_beta) + Caq_beta
