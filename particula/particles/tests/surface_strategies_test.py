...
def test_molar_surface_strategy():
    ...
    assert strategy.effective_surface_tension(mass_concentration) == pytest.approx(
...
def test_mass_surface_strategy():
    ...
    # Test effective surface tension
    mass_concentration = np.array([100, 200])  # water, oil
    expected_surface_tension = surface_tension[0] * 100 / (100 + 200) + surface_tension[
        1
    ] * 200 / (100 + 200)
    assert strategy.effective_surface_tension(mass_concentration) == pytest.approx(
        expected_surface_tension
    ...
    # Test effective density
    expected_density = density[0] * 100 / (100 + 200) + density[1] * 200 / (100 + 200)
    assert strategy.effective_density(mass_concentration) == pytest.approx(
        expected_density
    )
    ...
    assert strategy.kelvin_radius(0.01815, mass_concentration, 298) == pytest.approx(
        expected_kelvin_radius, rel=1e-6
    )
