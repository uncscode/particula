...
def test_cunningham_slip_correction_basic():
    ...
    # previous method
    knudsen_number = 0.5
    expected = 1 + knudsen_number * (1.257 + 0.4 * np.exp(-1.1 / knudsen_number))
...
def test_cunningham_slip_correction_array():
    ...
    expected = 1 + knudsen_numbers * (1.257 + 0.4 * np.exp(-1.1 / knudsen_numbers))
    np.testing.assert_allclose(cunningham_slip_correction(knudsen_numbers), expected)
